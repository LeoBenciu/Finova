                ...
                **Instructions**:
                ...
                2. **Extract Line Items**:
                   - Extract an array of line items (if present). For each item:
                     ...
                     - type: Article type (choose from: MARFURI, MATERII_PRIME, PRODUSE_FINITE, SEMIFABRICATE, DISCOUNT_FINANCIAL_IESIRI, MATERIALE_AUXILIARE, AMBALAJE, 
                     OBIECTE_DE_INVENTAR, AMENAJARI_PROVIZORII, MATERIALE_SPRE_PRELUCRARE, DISCOUNT_COMERCIAL_IESIRI, SERVICII_VANDUTE,
                     MATERIALE_IN_PASTRARE_SAU_CONSIGNATIE, DISCOUNT_FINANCIAL_INTRARI, COMBUSTIBILI, PIESE_DE_SCHIMB, ALTE_MATERIALE_CONSUMABILE, DISCOUNT_COMERCIAL_INTRARI, 
                     AMBALAJE_SGR, TAXA_VERDE).
                    ...
                    If EXISTING_ARTICLES is not empty, use the highest existing articleCode + 1 for the first new article, incrementing by 1 for each subsequent new article. 
                     ...
                     - vat: VAT rate (choose from: NINETEEN, NINE, FIVE, ZERO). If you can't find a VAT amount in the document for each line item that means that the VAT rate should be ZERO.
                     ...
                     - management: name of the management as string from the provided management list (select the most relevant).
                ...
                4. **Management Selection**:
                   - For each line item, select a management record from the provided management list that is most relevant (e.g., based on name or type).
