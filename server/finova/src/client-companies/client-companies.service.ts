...
export class ClientCompaniesService {
    ...
    async createClientCompany(ein:{ein:string}, reqUser: User, articlesFile: Express.Multer.File, managementFile: Express.Multer.File)
    {
        try
        {
            ...
            if(!articlesRows.length || !managementRows.length){
                throw new BadRequestException('CSV files cannot be empty!');
            };
            ...
            const articleData = articlesRows.map((row, index) => {
            ...
            return {
              ...
              const managementData = managementRows.map((row, index) => {
                const code = parseInt(row.cod);
                if (isNaN(code) || !row.cod) {
              ...
              const result = await this.prisma.$transaction(async (prisma) => {
                ...
                const managementResult = await prisma.management.createMany({
    ...
    private async validateCsvHeaders(file: Express.Multer.File, expectedColumns: string[]): Promise<void> {
        return new Promise((resolve, reject)=> {
            ...
            fs.createReadStream(file.path)
                ...
                .on('data', (row)=>{
                    if (!headersChecked){
                        if(!expectedColumns.every((col)=> Object.keys(row).includes(col))){
                            reject(
